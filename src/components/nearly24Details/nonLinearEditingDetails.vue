<!--非编近24小时详情-->
<template>
  <div class="no-edit content">
    <div class="navigation-cut clear">
      <div class="fl">
        <span></span>
        <span>近24小时非编业务详情</span>
      </div>
      <ul class="fr">
        <li>
          <router-link to="resourdeOption">总体资源消耗率</router-link>
        </li>
        <li>非编资源消耗</li>
        <li>近24小时非编业务详情</li>
      </ul>
    </div>

    <div class="table-details block-area">
      <!--筛选条件-->
      <div class="condition">
        <!--排序-->
        <div class="sort">
          <span class="sort-name">排序：</span>
          <span class="active time" @click="sortBySelectCondition($event,1)">开始时间<i class="icon" v-bind:class="isAscStartTime?'wb-arrow-up':'wb-arrow-down'"></i></span>
          <!--<span class="time" @click="sortBySelectCondition($event,2)">结束时间<i class="icon" v-bind:class="isAscEndTime?'wb-arrow-up':'wb-arrow-down'"></i></span>-->
          <!--<span class=" time" @click="sortBySelectCondition($event,3)">处理时长<i class="icon" v-bind:class="isDurationAsc?'wb-arrow-up':'wb-arrow-down'"></i></span>-->
        </div>

        <!--执行器名称-->
        <div class="name">
         <div class="sort-name">
           <i></i>
           <span>执行器名称</span>
         </div>
         <div class="select-down dropdown">
            <div>
              <input v-model="performerName"/>
              <span class="dropdown-toggle" data-toggle="dropdown"><i class="icon wb-triangle-down"></i></span>

              <ul class="no-header down-content dropdown-menu">
                <li @click="selectPerformerName(0,'全部')">全部</li>
                <li @click="selectPerformerName(1,item)" v-for="item in performerNameData">{{item}}</li>
              </ul>
            </div>

         </div>
        </div> <!--执行器名称  end-->

        <!--编辑人员-->
        <div class="redact-user">
          <div class="sort-name">
            <i></i>
            <span>编辑人员</span>
          </div>
        <div class="select-down dropdown">
          <div>
            <input v-model="redirectUser">
            <span  class="dropdown-toggle" data-toggle="dropdown"><i class="icon wb-triangle-down"></i></span>

            <ul class="has-header down-content dropdown-menu">
              <li @click="selectRedirectUser(0,'所有成员')">
                <span><img src="./../../assets/all-header.png"/></span>
                <span>所有成员</span>
              </li>
              <li  @click="selectRedirectUser(1,item)" v-for="item in listCensorUser">
                <span>{{item}}</span>
                <span>{{item}}</span>
              </li>
            </ul>
          </div>

        </div>
        </div><!--编辑人员 end-->

        <!--栏目-->
        <div class="programa">
          <div class="sort-name">
            <i></i>
            <span>栏目</span>
          </div>
          <div class="select-down dropdown">
            <div>
              <input v-model="programaName"/>
              <span  class="dropdown-toggle" data-toggle="dropdown"><i class="icon wb-triangle-down"></i></span>

              <ul class="no-header down-content dropdown-menu">
                <li @click="selectProgramaName(0,'全部')">全部</li>
                <li @click="selectProgramaName(1,item)" v-for="item in listProgramName">{{item}}</li>
              </ul>
            </div>

          </div>
        </div> <!--栏目  end-->

      </div><!--筛选条件  end-->
      <!--表格内容-->
      <div class="table-content">
        <table style="table-layout: fixed">
          <thead>
          <tr>
            <th class="name">任务名称</th>
            <th class="comm-time">开始时间</th>
            <th class="comm-time">结束时间</th>
            <th class="comm-duration">处理时长</th>
            <th class="comm-status">执行器名称</th>
            <th class="comm-user">编辑人员</th>
            <th class="comm-program">栏目</th>
          </tr>
          </thead>
        </table>
       <div class="scroll-table">
         <table style="table-layout: fixed">
           <tbody>
           <tr v-for="item in tableData">
             <td class="name">{{item.taskName}}</td>
             <td class="comm-time">{{item.startTime}}</td>
             <td class="comm-time">{{item.endTime}}</td>
             <td class="comm-duration">{{item.handTimes}}</td>
             <td class="comm-status">{{item.host}}</td>
             <td class="comm-user">{{item.username}}</td>
             <td class="comm-program">{{item.programName}}</td>
           </tr>
           </tbody>
         </table>
       </div>

      </div><!--表格内容  end-->

      <!--页码-->
      <div class="page clear">
        <div class="cut-page-number fl">
          <div>每页条数</div>
          <div class="select-down dropdown">
            <div>
              <input v-model="pageSize"/>
              <span  class="dropdown-toggle" data-toggle="dropdown"><i class="icon wb-triangle-down"></i></span>
              <ul class="page-number down-content dropdown-menu">
                <li @click="selectPageNumber(15)">15</li>
                <li @click="selectPageNumber(20)">20</li>
                <li @click="selectPageNumber(25)">25</li>
                <li @click="selectPageNumber(30)">30</li>
              </ul>
            </div>

          </div>
          <div>总条数 {{total}}，共 {{totalPages}} 页</div>
        </div>

        <div class="page-number fr">
          <div class="page-turning" id="paging"></div>
          <div class="go-page">
            <span>
              <input type="number" v-model="goToPage" @keyup="changeBorderColor()"/>
            </span>
            <span @click="appointToPage()">GO</span>
          </div>
        </div>

      </div>
    </div>
  </div>
</template>

<script>
  import {GlobalVar} from '../../../static/js/utils'
  import {DateUtils} from '../../../static/js/DateUtils'

  export default {
    name:'noEdit',
    data(){
      return{
        performerName:'全部',
        redirectUser:'所有成员',
        programaName:'全部',
        isDurationAsc:true, //处理时长是否是 升序
        isAscStartTime:true,//开始时间是否是 升序
        isAscEndTime:true,//结束时间是否是 升序
        listCensorUser:[],//编辑人员
        listProgramName:[],//栏目名称
        performerNameData:[],//执行器名称
        tableData:[],
        goToPage:1,
        businessSn:'',
        businessType:'',
        totalPages:1,//总页数
        total:'',//总条数
        pageNum:1,//当前页码
        pageSize:15,//每页显示的个数
        objFormate:"",//目标格式
        ascDesc:'asc',
        sortField:'startTime',//选择的条件
        selectProgramName:'',//节目名称
        examineUser:'',//审查人
        performer:'',//执行器名称
        selectUrl:'&first=&second=&third=',
      }
    },
    methods:{
      /*  执行器名称  */
      selectPerformerName:function (flag,name) {
        var self = this;
        this.performerName = name;
        if(Number(flag) == 1){
          this.performer = name;
        }else{
          this.performer = '';
        }
        this.selectUrl = '&first='+this.performer+'&second='+this.examineUser+'&third='+this.selectProgramName;
        getAllData(self,this.selectUrl);
        },
      /*编辑人员*/
      selectRedirectUser:function (flag,name) {
        var self = this;
        this.redirectUser = name;
        if(Number(flag) == 1){
          this.examineUser = name;
        }else{
          this.examineUser = '';
        }
        this.selectUrl = '&first='+this.performer+'&second='+this.examineUser+'&third='+this.selectProgramName;
        getAllData(self,this.selectUrl);
        },
      /*栏目*/
      selectProgramaName:function (flag,name) {
        var self = this;
        this.programaName = name;
        if(Number(flag) == 1){
          this.selectProgramName = name;
        }else{
          this.selectProgramName = '';
        }
        this.selectUrl = '&first='+this.performer+'&second='+this.examineUser+'&third='+this.selectProgramName;
        getAllData(self,this.selectUrl);
      },
      sortBySelectCondition:function (event,flag) {
        var self = this;
        $(event.target).parents('div.sort').find('span.time').removeClass('active');
        $(event.target).addClass('active');
        //开始时间：starttime,结束时间：finishtime,处理时长:handletimeS
        //按照选择的条件排序
        //升序asc 降序desc
        if(Number(flag) == 1){
          //开始时间
          this.isAscStartTime = !this.isAscStartTime;
          this.sortField = 'startTime';
          this.ascDesc = this.isAscStartTime?'asc':'desc';
        }else if(Number(flag) == 2){
          //结束时间
          this.isAscEndTime = !this.isAscEndTime;
          this.sortField = 'endTime';
          this.ascDesc = this.isAscEndTime?'asc':'desc';
        }else{
          //处理时长
          this.isDurationAsc = !this.isDurationAsc;
          this.sortField = 'handletimeS';
          this.ascDesc = this.isDurationAsc?'asc':'desc';
        }
        getAllData(self,this.selectUrl);
      },
      selectPageNumber:function (falg) {
        var self = this;
        self.pageNum = 1;
        this.pageSize = falg;
        getAllData(self,this.selectUrl);
      },
      appointToPage:function () {
        //指定到某一页面
        //totalPages  goToPage
        var self = this;
        if(this.goToPage>this.totalPages || this.goToPage<=0){
          $('.page .page-number>div.go-page').css('border-color','#ff6c60');
        }else{
          self.pageNum = self.goToPage;
          //调用接口
          getAllData(self,this.selectUrl);
        }
      },
      changeBorderColor:function () {
        $('.page .page-number>div.go-page').css('border-color','#cccccc');
      }
    },
    updated:function () {
      GlobalVar.setScroll('down-content');
    },
    mounted:function () {
      var self = this;
      //得到传送过来的值
      var receiveBusiness = GlobalVar.getSessionStorageJson('non-linear-editing');
      if(!GlobalVar.isEmptyObject(receiveBusiness)){
        self.programaName = receiveBusiness.programaName;
        self.businessSn = receiveBusiness.businessSn;
        self.businessType = receiveBusiness.businessType;
      }
      var router = {
        url: this.$route.path,
        name: this.$route.name,
        className: 'active'
      }
      GlobalVar.setHistoryRouter(router);
      getAllData(self,self.selectUrl);
      getUserList(self);
    }
  }


  /*
  * first 执行器名称
  * second 编辑人员
  * third 栏目
  * sortField 排序字段 handletimeS  startTime，endTime
  * ascDesc 升序asc 降序desc
  * pageNum 当前页数
  * pageSize 页面大小
  *  ///consumeBusTotal/busTaskInfo/censor
  * */
  function getAllData(self,selectUrl) {
    $.ajax({
      type:'get',
      url:GlobalVar.urlNameW +'consumeBusTotal/busTaskInfo/'+self.businessSn+'?ascDesc='+self.ascDesc+'&pageNum='+self.pageNum+'&pageSize='+self.pageSize+
      '&sortField='+self.sortField+selectUrl,
      success:function(result){
        if(Number(result.status) == 200){
          //pageNum  pageSize  pages  total
          self.totalPages = result.data.pages;
          self.total = result.data.total;
          self.tableData  = result.data.rows;
          pageNumber(self.totalPages,self.pageNum,self);
        }
      },error:function (e) {}
    })
  }

  /*
 * 得到审片人和节目名称
 * busTaskInfo/censorFilterList
 * */
  function getUserList(self) {
    $.ajax({
      type:'get',
      url:GlobalVar.urlNameW +'consumeBusTotal/busTaskInfo/editFilterList',
      success:function (result) {
        if(Number(result.status) == 200){
          self.listCensorUser = result.data.censorUser;
          self.listProgramName = result.data.programName;
          self.performerNameData = result.data.hosts;
          //hosts
        }
      },error:function (e) {}
    })

  }


  //页码
  function pageNumber(pageCount,current,self) {
    $("#paging").CreatePage({
      pageCount: pageCount,//总页数
      current: current,//当前页码
        backFn: function (page) {
          self.pageNum = page;
          //page:点击时返回的点击的页码，拿到该页码过后执行翻页的逻辑操作
          getAllData(self,self.selectUrl);
        }
      });
  }
</script>


<style scoped>
  @import "../../../static/css/nearly24-details.css";
  .page .page-number>div.go-page>span input{
    width: 100%;
    border: none;
    border-radius: 4px;
    height: 24px;
    padding-left: 10px;
  }
  .page div.select-down ul.down-content{
    top:-117px;
  }
  .dropdown-menu{
    min-width:55px;
  }

  .condition .sort i{
    display: none;
  }
  .condition .sort>span.active i{
    display: inline-block;
  }

  .condition .sort>span.active{
    background-color: #18d96e;
    color: #ffffff;
  }
  .table-content th.name,
  .table-content td.name{
    width: 20%;
  }

  .table-content th.comm-time,
  .table-content td.comm-time{
    width: 15%;
  }

  .table-content th.comm-duration,
  .table-content td.comm-duration{
    width: 10%;
  }

  .table-content th.comm-program,
  .table-content td.comm-program{
    width: 15%;
  }

  .table-content th.comm-status,
  .table-content td.comm-status{
    width: 15%;
  }

  .table-content th.comm-user,
  .table-content td.comm-user{
    width: 10%;
  }

</style>
